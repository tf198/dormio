...
/**
 * Schema generation from meta definitions
 *
...
class Dormio_Schema {
  ...
  public static function factory($lang, $spec) {
    ...
    if ($spec instanceof Dormio_Model)
      $spec = self::getSchema($spec->_meta);
    if ($spec instanceof Dormio_Meta)
      $spec = self::getSchema($spec);
    if (is_string($spec))
      $spec = self::getSchema(Dormio_Meta::get($spec));
    if ($lang instanceof PDO)
      $lang = $lang->getAttribute(PDO::ATTR_DRIVER_NAME);
  ...
  public static function getSchema($meta) {
    $schema = $meta->_spec;
    $schema['columns'] = array();
    foreach($schema['fields'] as $key=>$spec) {
      if(isset($spec['is_field']) && $spec['is_field']) $schema['columns'][$key] = $spec;
    }
...
class Dormio_Schema_Generic implements Dormio_Schema_Driver {
  ...
  public function __construct($spec) {
    ...
    if (!isset($spec['columns']))
      throw new Dormio_Schema_Exception("Require 'columns' key");
    $this->spec = $spec;
  }
