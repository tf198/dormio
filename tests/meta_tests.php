...
class TestOfMeta extends UnitTestCase{
  ...
  function testComment() {
    ...
    $intermediate = Dormio_Meta::get('comment_x_tag');
  ...
  function testResolve() {
    ...
    $this->assertEqual($spec, array('type'=>'manytomany', 'model'=>'module', 'verbose'=>'Depends On', 'through'=>'module_x_module', 'map_local_field'=>'l_module', 'map_remote_field'=>'r_module'));
    
    // reverse manytomany onto self
    $spec = $module->getSpec('module_set');
    $this->assertEqual($spec, array('type'=>'manytomany', 'model'=>'module', 'through'=>'module_x_module', 'map_local_field'=>'r_module', 'map_remote_field'=>'l_module'));
    
  }
  
  function testReverseSpec() {
    $module = Dormio_Meta::get('module');
    $through = Dormio_Meta::get('module_x_module');
    
    $this->assertEqual($through->getReverseSpec('module', 'l_module'), array('type'=>'foreignkey_rev', 'local_field'=>'pk', 'remote_field'=>'l_module', 'model'=>'module_x_module', 'on_delete'=>'cascade'));
    $this->assertEqual($through->getReverseSpec('module', 'r_module'), array('type'=>'foreignkey_rev', 'local_field'=>'pk', 'remote_field'=>'r_module', 'model'=>'module_x_module', 'on_delete'=>'cascade'));
    
    try {
      $through->getReverseSpec('Bad_Class', null);
    } catch(Dormio_Meta_Exception $dme) {
      $this->assertEqual($dme->getMessage(), "No reverse relation for 'bad_class' on 'module_x_module'");
    }
    
    try {
      $through->getReverseSpec('module', 'bad_accessor');
    } catch(Dormio_Meta_Exception $dme) {
      $this->assertEqual($dme->getMessage(), "No reverse accessor 'module.bad_accessor' on 'module_x_module'");
    }
